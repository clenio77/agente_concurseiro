...
def run_crew(cargo, concurso, banca, cidade, study_hours, study_months):
    ...
    search_task = Task(
        description=(
            f"Buscar provas anteriores para o cargo '{cargo}' no concurso "
            f"'{concurso}', organizado pela banca '{banca}' na cidade '{cidade}'."
        ),
        agent=search_agent,
    ...
    study_plan_task = Task(
        description=(
            f"Criar um plano de estudos para {study_hours} horas/semana durante "
            f"{study_months} meses com base nas provas encontradas."
        ),
    ...
    mock_exam_task = Task(
        description=(
            f"Gerar um simulado com base nos padrões da banca '{banca}' e nas "
            "provas encontradas."
        ),
    ...
    spaced_repetition_task = Task(
        description=(
            "Criar um plano de revisão espaçada para os tópicos do plano de "
            "estudos, otimizando a memorização."
        ),
    ...
    performance_prediction_task = Task(
        description=(
            "Prever o desempenho futuro com base no plano de estudos e dados "
