...
def render_simulado_page():
    ...
    with st.sidebar:
        ...
        difficulty = st.select_slider(
            "Dificuldade", options=["Fácil", "Média", "Difícil"], value="Média"
        )

        # Mapeamento de dificuldade
        difficulty_map = {"Fácil": "easy", "Média": "medium", "Difícil": "hard"}

        # Número de questões
        num_questions = st.slider(
            "Número de questões", min_value=5, max_value=30, value=10, step=5
        )
    ...
    if st.session_state.current_quiz and not st.session_state.quiz_submitted:
        ...
        with st.form("quiz_form"):
            ...
            for i, question in enumerate(quiz["questions"]):
                ...
                st.markdown(
                    f"*{question['subject']} - {question['banca']} ({question['year']})*"
                )
                ...
                selected_option = st.radio(
                    ...
                    index=(
                        ...
                        else list(options.keys()).index(current_answer)
                    ),
                )
            ...
            if submit_button:
                ...
                if len(st.session_state.quiz_answers) < len(quiz["questions"]):
                    st.warning(
                        f"Atenção: Você respondeu apenas "
                        f"{len(st.session_state.quiz_answers)} de "
                        f"{len(quiz['questions'])} questões. Tem certeza que deseja finalizar?"
                    )
    ...
    # Exibir resultados do simulado
        ...
        with col2:
            st.metric(
                "Acertos", f"{results['correct_count']}/{len(quiz['questions'])}"
            )
        with col3:
        ...
        if "subject_scores" in results and results["subject_scores"]:
            ...
            # Criar DataFrame
            df = pd.DataFrame({"Matéria": subjects, "Pontuação": scores})
            ...
            chart = (
                ...
                .properties(height=30 * len(subjects))
            )
        ...
        for i, question_result in enumerate(results["question_results"]):
            ...
            question = next(
                (q for q in quiz["questions"] if q["id"] == question_result["question_id"]),
                None,
            )
            ...
            with st.expander(f"Questão {i+1}: {result_icon} {question['text'][:50]}..."):
                ...
                for opt in question["options"]:
                    ...
                    if opt["id"] == question_result["correct_answer"]:
                        ...
                        opt["id"] == question_result["submitted_answer"]
                        and not question_result["is_correct"]
                    ):
                        st.markdown(f"**{opt['id']}) {opt['text']} ✗**")
                    else:
                        st.markdown(f"{opt['id']}) {opt['text']}")
    ...
    # Exibir página inicial
        ...
        # Criar DataFrame
        df = pd.DataFrame({"Data": dates, "Pontuação": scores})
